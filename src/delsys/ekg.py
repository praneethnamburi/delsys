...
class EKG(pysampled.Data):
    ...
    +-------------------------+-----------------------------------------------+
    | ``rpeaks_idx_ectopic``  | Peaks LABELLED ectopic. A classification of a |
    |                         | real beat, not a removal: the beat happened,  |
    |                         | it just is not sinus. Excluded/corrected      |
    |                         | downstream, and countable as a burden.        |
    +-------------------------+-----------------------------------------------+
    ...
    def rpeaks_decision(self) -> Dict[str, Any]:
        ...
        Returns:
            ``{"detector", "added", "removed", "ectopic", "flipped", "tags"}`` where ``added``
    ...
    def _times_to_final_peak_idx(self, times: List[float], tol: float = 0.1) -> List[int]:
        ...
        An ectopic label classifies a beat that is *present* in the result, so it snaps to the
        curated peak set (default + added - removed), unlike a removal, which references a
        detector default. A label with no peak within ``tol`` is dropped rather than allowed to
        attach itself to an unrelated beat."""
        final = list(self._get_rpeaks_from_meta())
        if not final or not times:
            return []
        ft = self.t[np.array(final)]
        out = []
        for x in times:
            j = int(np.argmin(np.abs(ft - float(x))))
            if abs(float(ft[j]) - float(x)) <= tol:
                out.append(int(final[j]))
        return sorted(set(out))

    def ectopic_times(self) -> np.ndarray:
        """Times (s) of peaks labelled ectopic."""
    ...
    def detect_ectopics(self, short: float = 0.85, tol: float = 0.20, local: int = 5,
        ...
        for i in range(len(rr) - 1):
            if med[i] <= 0:
                continue
            if rr[i] < short * med[i] and abs((rr[i] + rr[i + 1]) / (2 * med[i]) - 1.0) < tol:
                hit.append(int(peaks[i + 1]))          # the PREMATURE beat ends interval i
        cur = set(self.meta.get("rpeaks_idx_ectopic", []))
